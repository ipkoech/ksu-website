...
The portal selector now routes users to the consolidated workspaces below. Access is resolved from assigned permissions, including legacy governance, institutional administration, corporate communication, content, media, and homepage permissions.

| Current portal | URL | Compatible legacy route |
| --- | --- | --- |
| Admin Portal | `/admin` | `/governance`, `/institutional-administration` |
| CoCMS | `/cocms` | `/corporate-communication` |
| Schools | `/schools` | None |
| Departments | `/departments` | None |
| Research | `/research` | None |
| Library | `/library` | None |
| Publications | `/publications` | None |
| Student Clubs | `/student-clubs` | None |

The old admin routes remain redirects only; they do not render their retired portal shells. Seeded browser-QA accounts cover Super Admin, Admin, CoCMS, Research, Library, School, Department, Publications, and Student Clubs roles. Legacy permission names remain active so existing users continue to receive their mapped Admin or CoCMS access.

## Portal User Seeding

The main-service seed runner creates browser-QA portal users. Set a non-production password before running it; the seed intentionally fails when `KSU_SEED_PORTAL_USER_PASSWORD` is unset.

```bash
export KSU_SEED_PORTAL_USER_PASSWORD='replace-with-a-local-development-password'
PYTHONPATH=services/main:services/common services/main/.venv/bin/python -m app.seeders.seed_runner
```

Run the command from the repository root with the target database environment configured. The Student Clubs QA account is assigned to one seeded club and therefore receives locked, club-scoped access.
