...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main id="research-main" className="min-h-screen bg-white">
      <ResearchDetailHero eyebrow="Research Service" title={service.name ?? "Research service"} body={compactText(service.summary) || compactText(service.description)} breadcrumbs={[{ label: "Home", href: "/" }, { label: "Services", href: "/services" }, { label: service.name ?? "Service" }]} labels={[service.service_type, service.category, service.is_free ? "free" : null]} facts={[{ label: "Turnaround", value: service.turnaround_time }, { label: "Cost", value: service.is_free ? "Free" : compactText(service.fee_structure) }, { label: "Email", value: service.contact_email }, { label: "Attachments", value: attachments.length }]} actions={[{ label: "Back to services", href: "/services", variant: "secondary" }, ...(compactText(service.request_url) ? [{ label: "Request service", href: compactText(service.request_url) }] : [])]} imageSrc="/images/research/registrar-reirm-imagegen.png" imageAlt="Research service access and request information" />
