...
import {
  researchNewsFallbackImages as fallbackImages,
  researchNewsTabs as navButtons,
} from "../../config/research-page-content";
...
import {
  NewsEventsCalendar,
  type NewsEventDto,
} from "../../components/news-events-calendar";
...
export default async function NewsPage({
  ...
  const gallery = combined
    .filter((item) => getRecordImage(item.record))
    .slice(0, 8);
  ...
  const activeTab =
    requestedTab && availableTabs.some((button) => button.id === requestedTab)
      ? requestedTab
      : undefined;
  ...
  const eventRows: NewsEventDto[] = calendarItems.map((item, index) => ({
    id: String(item.record.id ?? item.record.slug ?? index),
    title: getRecordTitle(item.record, "Research event"),
    href: item.href ?? "/news?tab=events",
    date: recordDate(item.record) ? String(recordDate(item.record)) : null,
    venue:
          ...
          {showAll ? (
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1.9fr)_minmax(360px,0.95fr)]">
              ...
              <aside className="min-w-0 space-y-8">
                {calendarItems.length > 0 ? (
                  <NewsEventsCalendar items={eventRows} />
                ) : null}
            ...
            <div className="space-y-8">
              {activeTab === "news" && newsItems.length > 0 ? (
                <LatestResearch items={newsItems.slice(0, 9)} title="News" />
              ) : null}
              {activeTab === "articles" && articleItems.length > 0 ? (
                <LatestResearch
                  items={articleItems.slice(0, 9)}
                  title="Articles"
                />
              ) : null}
              {activeTab === "events" && calendarItems.length > 0 ? (
                <NewsEventsCalendar items={eventRows} />
              ) : null}
              {activeTab === "announcements" && noticeItems.length > 0 ? (
                <AnnouncementsPanel items={noticeItems} />
              ) : null}
              {activeTab === "gallery" && gallery.length > 0 ? (
                <ResearchGallery items={gallery} flush />
              ) : null}
            </div>
          )}
  ...
  return (
    <>
      <ResearchPageHero
        title="News & Events"
        eyebrow="Updates"
        description="News, insights, upcoming events, and announcements from Kisii University."
      ...
      <ResearchPageSummary actions={[]}>
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(360px,520px)] lg:items-center">
          ...
          <form
            action="/news"
            className="flex rounded-md border border-white/20 bg-white/95 p-1 shadow-xl"
          >
            {activeTab ? (
...
function StoryCard({
  ...
  const content = (
    <article className="group overflow-hidden rounded-lg border border-border bg-card shadow-sm transition hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md">
      ...
      <div className="p-4">
        <h2
          className={`${priority ? "text-xl" : "text-lg"} line-clamp-2 font-display font-semibold leading-7 text-foreground`}
        >
...
function AnnouncementsPanel({ items }: { items: ContentItem[] }) {
  ...
  return (
    <div id="announcements">
      <ScrollReveal
        className="space-y-4 rounded-lg bg-surface-subtle/80 p-5"
        variant="fade-up"
      >
        ...
        <div className="overflow-hidden rounded-lg border border-border bg-white shadow-sm">
          {items.map((item) => (
            <article
              key={item.record.id}
              className="grid grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-3 border-b border-border p-4 last:border-b-0"
            >
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs font-semibold text-muted-foreground">
                  {compactText(item.record.category) || "Research"}{" "}
                  {formatDate(item.record.valid_to)
                    ? ` • Due ${formatDate(item.record.valid_to)}`
                    : ""}
                </p>
              </div>
...
function ResearchGallery({
  ...
  return (
    <div id="gallery">
      <ScrollReveal
        className={`${flush ? "" : "mt-8"} space-y-4`}
        variant="fade-up"
      >
        <SectionHeader title="Gallery" href="/news#gallery" />
...
function ResearchUpdatesBand() {
  return (
    <ScrollReveal
      className="mt-8 overflow-hidden rounded-lg border border-primary/15 bg-[linear-gradient(135deg,hsl(var(--brand-overlay)),hsl(var(--primary)/.62))] text-white shadow-sm"
      variant="fade-up"
    >
      <div className="grid gap-5 p-6 lg:grid-cols-[minmax(260px,0.9fr)_minmax(320px,0.9fr)_minmax(420px,1.2fr)] lg:items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-white/80">
              Subscribe to receive the latest research news, articles, events
              and announcements from Kisii University.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <UpdateBenefit
            ...
            body="Be the first to know"
          />
          <UpdateBenefit
            icon={TrendingUp}
            title="Curated insights"
            body="Research that matters"
          />
          <UpdateBenefit
            icon={SparkIcon}
            title="Make an impact"
            body="Support our mission"
          />
        </div>
      </div>
    </ScrollReveal>
...
function KindBadge({
  ...
  return (
    <span
      className={`${className} rounded-md px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider ${tone}`}
    >
...
function MetaLine({
  ...
  return (
    <div
      className={`${className} flex flex-wrap items-center gap-2 text-xs font-semibold text-muted-foreground`}
    >
      {date ? (
        <>
...
function normalizeTab(value?: string) {
  ...
  return navButtons.some((button) => button.id === normalized)
    ? (normalized as NewsTab)
    : undefined;
}
...
function normalizeArticleKind(record: ResearchGenericRecord): ContentKind {
  const type = compactText(
    record.article_type ?? record.news_type,
  ).toLowerCase();
