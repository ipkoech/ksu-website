...
function CommunityImpactHero({
  ...
  return (
    <section className="relative overflow-hidden border-b border-border bg-[hsl(var(--surface-subtle))] px-4 py-8 text-foreground sm:px-6 lg:px-8 xl:px-10 2xl:px-12">
      ...
      <div className="relative mx-auto grid min-h-[260px] max-w-[1680px] gap-6 lg:grid-cols-[minmax(0,0.45fr)_minmax(460px,0.55fr)] lg:items-center">
        ...
        <div className="relative z-10 flex justify-end">
          {stats.length > 0 ? (
            <dl className="w-full max-w-[240px] rounded-lg border border-border bg-white/90 p-4 shadow-[0_22px_70px_-46px_rgba(15,23,42,0.48)] backdrop-blur">
              <dt className="mb-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Impact highlights</dt>
              {stats.map((stat) => {
                const Icon = stat.icon;
                return (
                  <div key={stat.label} className="flex items-center gap-3 border-t border-border py-3 first:border-t-0 first:pt-0 last:pb-0">
                    <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-primary">
                      <Icon aria-hidden className="h-4 w-4" />
                    </span>
                    <div>
                      <dt className="text-xs font-medium text-muted-foreground">{stat.label}</dt>
                      <dd className="text-lg font-semibold text-foreground">{stat.value.toLocaleString()}{stat.value > 999 ? "+" : ""}</dd>
                    </div>
                  </div>
                );
              })}
        ...
        return (
          <article key={item.label} className="grid gap-3 p-4 sm:grid-cols-[52px_minmax(0,1fr)]">
            ...
            <div>
              ...
              <p className="mt-1 line-clamp-2 text-sm leading-6 text-muted-foreground">{item.value}</p>
            </div>
          </article>
        );
      })}
    ...
    <section className="rounded-lg border border-border bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => {
          ...
          return (
            <article key={card.id} className="rounded-lg border border-border bg-white p-4 text-center">
              <Icon aria-hidden className="mx-auto h-8 w-8 text-primary" />
              <h3 className="mt-3 text-sm font-medium leading-5 text-muted-foreground">{card.title}</h3>
              {card.value ? <p className="mt-3 text-3xl font-semibold text-primary">{card.value}</p> : null}
              {card.body ? <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{card.body}</p> : null}
            </article>
          );
        })}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(220px,0.8fr)_minmax(0,1fr)]">
        ...
        <div className="grid gap-2">
          {featuredEvents.map((event) => {
            ...
            return (
              <Link key={compactText(event.id) || compactText(event.slug) || getRecordTitle(event, "Research event")} href={event.slug ? `/events/${event.slug}` : "/news?tab=-events"} className="grid grid-cols-[64px_minmax(0,1fr)] gap-3 rounded-lg border border-border p-3 transition hover:border-primary/35">
                ...
                <div className="min-w-0">
                  <h3 className="line-clamp-2 text-sm font-semibold leading-5 text-foreground">{getRecordTitle(event, "Research event")}</h3>
      ...
      <div className="mt-3 divide-y divide-border">
        {quickLinks.map((link) => (
          <Link key={link.href} href={link.href} className="group flex items-center justify-between gap-4 py-3">
            ...
            <ArrowRight aria-hidden className="h-4 w-4 shrink-0 text-muted-foreground/70 transition group-hover:translate-x-1 group-hover:text-primary" />
          </Link>
        ))}
      </div>
    </aside>
    ...
    <svg viewBox="0 0 760 360" className="h-full w-full" role="img" aria-label="Community impact illustration">
      <defs>
        <linearGradient id="community-sky" x1="0" x2="1">
          <stop offset="0" stopColor="hsl(var(--surface-subtle))" />
          <stop offset="1" stopColor="hsl(var(--surface-muted))" />
        </linearGradient>
      </defs>
      <rect width="760" height="360" fill="url(#community-sky)" opacity="0.2" />
      <path d="M0 230 C120 170 210 196 316 142 C444 77 560 94 760 46 V360 H0Z" fill="hsl(var(--surface-muted))" />
      <path d="M0 282 C128 236 230 256 348 210 C480 158 606 168 760 118 V360 H0Z" fill="hsl(var(--surface-muted))" />
      <path d="M90 286 C180 242 280 244 380 214 C512 176 612 174 736 140" fill="none" stroke="hsl(var(--primary))" strokeOpacity="0.24" strokeWidth="2" />
      <path d="M510 108 L642 70 L720 132 L684 240 L548 222 L486 154Z" fill="hsl(var(--surface-subtle))" stroke="hsl(var(--primary))" strokeWidth="2" />
      <path d="M578 116 L640 104 L686 138 L664 198 L596 190 L558 146Z" fill="hsl(var(--primary))" opacity="0.78" />
      <path d="M260 250 V176 C260 144 286 118 318 118 H374 C406 118 432 144 432 176 V250Z" fill="hsl(var(--surface-muted))" stroke="hsl(var(--primary))" strokeOpacity="0.3" />
      <rect x="300" y="178" width="92" height="72" fill="#ffffff" opacity="0.7" />
      <circle cx="190" cy="154" r="32" fill="hsl(var(--primary))" />
      <path d="M156 220 C162 176 218 176 224 220Z" fill="hsl(var(--primary))" />
      <circle cx="262" cy="160" r="30" fill="hsl(var(--secondary))" />
      <path d="M232 226 C238 184 290 184 298 226Z" fill="hsl(var(--primary))" />
      <circle cx="332" cy="160" r="31" fill="#263b4d" />
      <path d="M300 228 C308 184 358 184 368 228Z" fill="hsl(var(--primary))" />
      <rect x="182" y="202" width="126" height="78" rx="14" fill="hsl(var(--surface-muted))" stroke="hsl(var(--primary))" strokeOpacity="0.22" />
      <rect x="470" y="120" width="112" height="128" rx="14" fill="#ffffff" opacity="0.76" stroke="hsl(var(--primary))" strokeOpacity="0.18" />
      <path d="M512 210 C518 176 546 174 552 210Z" fill="hsl(var(--primary))" opacity="0.82" />
      <circle cx="532" cy="166" r="18" fill="hsl(var(--primary))" />
      <path d="M642 104 C666 88 694 96 710 120" fill="none" stroke="hsl(var(--primary))" strokeOpacity="0.42" strokeWidth="2" />
