  ...
  return (
    <label className="grid gap-1 text-xs font-bold uppercase tracking-[0.08em] text-slate-500">
      ...
      <select
        ...
        className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold normal-case tracking-normal text-slate-700 shadow-sm outline-none transition focus:border-primary/50 focus:ring-2 focus:ring-primary/10"
      >
        {options.map((option) => (
          ...
          {visibleRecords.map((record, index) => {
            ...
            return (
              <article
                ...
                className="flex min-h-[15rem] flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-md"
              >
                ...
                <h3 className="mt-4 text-sm font-bold leading-6 text-slate-950">
                  {record.title}
                </h3>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={safePage === 1}
              className="inline-flex h-10 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700 transition hover:border-primary/30 hover:text-primary disabled:cursor-not-allowed disabled:opacity-40"
            >
      ...
      {records.map((record, index) => {
        ...
        return (
          <article
            ...
            className="grid gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-[minmax(0,1fr)_220px]"
          >
            ...
            <div className="grid content-start gap-2 sm:grid-cols-2 lg:grid-cols-1">
              ...
              {meta
                ...
                .map((item) => (
                  <span
                    key={item}
                    className="rounded-lg bg-slate-50 px-3 py-2 text-xs font-bold uppercase tracking-[0.06em] text-slate-600"
                  >
                    {item}
                  </span>
                ))}
          ...
          <li key={`${record.title}-${index}`} className="relative">
            ...
            <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <h3 className="max-w-3xl text-sm font-bold leading-6 text-slate-950">
                  {record.title}
                </h3>
                {year ? (
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {records.map((record, index) => (
        <article
          key={`${record.title}-${index}`}
          className="rounded-xl border border-slate-200 bg-[linear-gradient(135deg,#ffffff_0%,#f8fbff_100%)] p-4 shadow-sm"
        >
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {recordMeta(record)
                ...
                .map((item) => (
                  <span
                    key={item}
                    className="rounded-full bg-white px-2.5 py-1 text-[0.68rem] font-bold uppercase tracking-[0.06em] text-slate-600 ring-1 ring-slate-200"
                  >
                    {item}
                  </span>
                ))}
