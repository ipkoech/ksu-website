        ...
        <div className="relative mt-11 grid gap-8 md:grid-cols-2 lg:grid-cols-4 lg:gap-4">
          ...
          {section.items.map((item, index) => (
            <article key={item.id} className="group relative text-center">
              <span className={`relative z-10 mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border-2 bg-white shadow-[0_0_0_8px_white] transition-[transform,background-color,color] duration-200 group-hover:-translate-y-1 group-hover:text-white motion-reduce:transform-none ${index % 2 === 0 ? "border-primary text-primary group-hover:bg-primary" : "border-secondary text-secondary group-hover:bg-secondary"}`}>
    ...
    <section className="overflow-hidden bg-white px-5 py-14 sm:px-8 lg:px-10 lg:py-20">
      <AboutReveal className="mx-auto max-w-7xl">
        ...
        <div className="relative mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-5 lg:gap-0">
          ...
          {section.items.map((item, index) => (
            <article key={item.id} className="group relative flex flex-col border border-primary/10 bg-white p-4 transition-[transform,border-color,box-shadow] duration-200 hover:-translate-y-1 hover:border-secondary hover:shadow-xl motion-reduce:transform-none lg:border-y-0 lg:border-l-0 lg:border-r lg:px-4 lg:last:border-r-0">
              ...
              <ImageCurtainReveal className="mt-5 aspect-[4/3] rounded-2xl" direction={index % 2 === 0 ? "right" : "left"}>
                <Image src={pickImage(item.image?.url)} alt={item.image_alt_text || item.image?.alt_text || item.title} fill sizes="(min-width:1024px) 20vw, 50vw" className="object-cover transition-transform duration-500 motion-safe:group-hover:scale-[1.03] motion-reduce:transition-none" />
              </ImageCurtainReveal>
            </article>
          ))}
        </div>
      ...
      <AboutReveal className="mx-auto max-w-7xl">
        ...
        <div className="mx-auto mt-10 grid max-w-5xl gap-4 md:grid-cols-2">
          {documents.map((document) => (
            <article key={document.id} className="group flex min-h-52 flex-col rounded-2xl border border-primary/15 bg-[color-mix(in_srgb,hsl(var(--primary))_6%,white)] p-6 transition-[border-color,box-shadow] duration-200 hover:border-secondary hover:shadow-lg">
      ...
      <AboutReveal className="mx-auto grid max-w-7xl overflow-hidden md:grid-cols-[auto_1fr_auto] md:items-center" variant="scale">
        ...
        {href ? (
          <div className="px-6 pb-6 md:pb-0 md:pr-8">
            <Link href={href} className="inline-flex min-h-20 items-center gap-3 border border-secondary bg-white px-6 py-4 text-sm font-bold text-primary transition-colors duration-200 hover:bg-primary hover:text-white active:scale-[0.98]">
...
function ClosingCta({ type }: { type: PageType }) {
  ...
  return (
    <section className="relative overflow-hidden bg-primary px-5 py-14 text-white sm:px-8 lg:px-10 lg:py-16">
      <div className="absolute -right-24 -top-24 h-72 w-72 rounded-2xl border border-white/10" aria-hidden />
      <div className="absolute -bottom-20 right-24 h-48 w-48 rounded-2xl border border-secondary/20" aria-hidden />
      <AboutReveal className="relative mx-auto flex max-w-7xl flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-secondary">Continue the journey</p>
        ...
        <div className="flex shrink-0 flex-wrap gap-3">
          {links.map((link, index) => (
            <Link key={link.href} href={link.href} className={`inline-flex min-h-12 items-center gap-2 px-5 py-3 text-sm font-bold transition-[transform,background-color] duration-200 hover:-translate-y-0.5 active:scale-[0.98] motion-reduce:transform-none ${index === 0 ? "bg-secondary text-foreground hover:bg-amber-400" : "border border-white/60 text-white hover:bg-white/10"}`}>
      ...
      <section className="relative isolate min-h-[430px] overflow-hidden bg-primary text-white">
        ...
        <div className="relative mx-auto flex min-h-[430px] max-w-7xl flex-col justify-center px-5 py-8 sm:px-8 lg:px-10">
          ...
          <div className="mt-8 max-w-3xl">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-secondary">{page.eyebrow}</p>
            <h1 className="mt-3 font-[family-name:var(--font-display)] text-4xl font-normal tracking-tight leading-[1.06] sm:text-5xl lg:text-[3.6rem]">
              {page.title}
            </h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-white/80">{page.introduction}</p>
            {!isCharter && primaryHref ? <Link href={primaryHref} className="mt-7 inline-flex min-h-12 items-center gap-2 bg-secondary px-5 py-3 text-sm font-bold uppercase text-foreground transition-colors duration-200 hover:bg-amber-400 active:scale-[0.98]"><Download className="h-4 w-4" aria-hidden />Download the Strategic Plan</Link> : null}
          </div>
        </div>
      </section>
