...
function SectionHeading({
  ...
  return (
    <div className={centered ? "mx-auto max-w-3xl text-center" : "max-w-2xl"}>
      ...
      <h2
        className={`mt-3 font-[family-name:var(--font-display)] text-3xl font-normal tracking-tight sm:text-4xl ${inverse ? "text-white" : "text-primary"}`}
      >
        {section.heading}
      </h2>
      {section.summary ? (
        <p
          className={`mt-4 leading-7 ${inverse ? "text-white/70" : "text-muted-foreground"}`}
        >
          {section.summary}
        </p>
      ) : null}
      ...
      <div className="mx-auto grid max-w-7xl lg:grid-cols-2 lg:items-stretch">
        ...
        >
          <Image
            ...
            alt={
              section.media_alt_text ||
              section.primary_media?.alt_text ||
              section.heading
            }
            fill
    ...
    <section className="bg-white px-5 py-14 sm:px-8 lg:px-10 lg:py-20">
      <AboutReveal className="mx-auto max-w-7xl">
        ...
        <div className="relative mt-11 grid gap-8 md:grid-cols-2 lg:grid-cols-4 lg:gap-4">
          <div
            className="absolute left-[12.5%] right-[12.5%] top-8 hidden h-px bg-primary/55 lg:block"
            aria-hidden
          />
          {section.items.map((item, index) => (
            <article key={item.id} className="group relative text-center">
              <span
                className={`relative z-10 mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border-2 bg-white shadow-[0_0_0_8px_white] transition-[transform,background-color,color] duration-200 group-hover:-translate-y-1 group-hover:text-white motion-reduce:transform-none ${index % 2 === 0 ? "border-primary text-primary group-hover:bg-primary" : "border-secondary text-secondary group-hover:bg-secondary"}`}
              >
...
function ProcessSection({ section }: { section: PublicInstitutionalSection }) {
  ...
  return (
    <section
      className={`${inverse ? "bg-primary text-white" : "bg-[#f5f2ea] text-foreground"} px-5 py-14 sm:px-8 lg:px-10 lg:py-20`}
    >
      <AboutReveal className="mx-auto max-w-7xl">
        ...
        <ol className="relative mt-12 grid gap-9 md:grid-cols-2 lg:grid-cols-4 lg:gap-6">
          <div
            className={`absolute left-[12.5%] right-[12.5%] top-7 hidden border-t border-dashed lg:block ${inverse ? "border-white/35" : "border-primary/35"}`}
            aria-hidden
          />
          {section.items.map((item, index) => (
            <li key={item.id} className="group relative text-center">
              <span
                className={`relative z-10 mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border-2 font-[family-name:var(--font-display)] text-lg font-semibold shadow-[0_0_0_7px_var(--step-ring)] transition-transform duration-200 group-hover:-translate-y-1 motion-reduce:transform-none ${inverse ? "[--step-ring:var(--primary)] border-secondary bg-primary text-secondary" : "[--step-ring:#f5f2ea] border-primary/30 bg-white text-primary"}`}
              >
                {String(index + 1).padStart(2, "0")}
              </span>
              <InstitutionalIcon
                name={item.icon_key}
                className={`mx-auto mt-5 h-6 w-6 ${inverse ? "text-secondary" : "text-primary"}`}
              />
              <h3 className="mt-3 font-[family-name:var(--font-display)] text-xl font-normal tracking-tight">
                {item.title}
              </h3>
              {item.description ? (
                <p
                  className={`mx-auto mt-2 max-w-xs text-sm leading-6 ${inverse ? "text-white/70" : "text-muted-foreground"}`}
                >
                  {item.description}
                </p>
              ) : null}
    ...
    <section className="overflow-hidden bg-white px-5 py-14 sm:px-8 lg:px-10 lg:py-20">
      <AboutReveal className="mx-auto max-w-7xl">
        ...
        <div className="relative mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-5 lg:gap-0">
          <div
            className="absolute left-0 right-0 top-9 hidden h-1 bg-secondary lg:block"
            aria-hidden
          />
          <ArrowRight
            className="absolute -right-1 top-[1.75rem] z-20 hidden h-6 w-6 text-secondary lg:block"
            aria-hidden
          />
          {section.items.map((item, index) => (
            <article
              key={item.id}
              className="group relative flex flex-col border border-primary/10 bg-white p-4 transition-[transform,border-color,box-shadow] duration-200 hover:-translate-y-1 hover:border-secondary hover:shadow-xl motion-reduce:transform-none lg:border-y-0 lg:border-l-0 lg:border-r lg:px-4 lg:last:border-r-0"
            >
              <span className="relative z-10 mx-auto flex h-[4.5rem] w-[4.5rem] items-center justify-center rounded-2xl border-2 border-secondary bg-white text-primary shadow-[0_0_0_8px_white] transition-colors duration-200 group-hover:bg-primary group-hover:text-white">
                <InstitutionalIcon name={item.icon_key} className="h-7 w-7" />
              </span>
              ...
              <h3 className="mt-2 font-[family-name:var(--font-display)] text-lg font-normal tracking-tight leading-snug text-primary">
                {item.title}
              </h3>
              {item.description ? (
              ...
              <ImageCurtainReveal
                className="mt-5 aspect-[4/3] rounded-2xl"
                direction={index % 2 === 0 ? "right" : "left"}
              >
                <Image
                  src={pickImage(item.image?.url)}
                  alt={
                    item.image_alt_text || item.image?.alt_text || item.title
                  }
                  fill
  ...
  return (
    <section className="overflow-hidden bg-[#f5f2ea]">
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[0.95fr_1.05fr] lg:items-stretch">
        <ImageCurtainReveal
          className="min-h-[340px] lg:min-h-[500px]"
          direction="left"
        >
          <Image
            src={pickImage(section.primary_media?.url)}
        ...
        <AboutReveal
          variant="right"
          className="px-5 py-14 sm:px-8 lg:px-12 lg:py-16"
        >
          <SectionHeading section={section} />
          <div className="mt-8 grid gap-6">
            {section.items.map((item, index) => (
              <article key={item.id} className="group flex gap-4">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-primary/20 text-primary">
                  <InstitutionalIcon name={item.icon_key} className="h-5 w-5" />
                </span>
                <div>
                  <p className="text-[0.65rem] font-bold uppercase tracking-[0.18em] text-secondary">
                    Action {String(index + 1).padStart(2, "0")}
                  </p>
                  <h3 className="mt-1 font-[family-name:var(--font-display)] text-lg font-normal tracking-tight text-primary">
                    {item.title}
                  </h3>
                  {item.description ? (
  ...
  return (
    <section className="bg-primary text-white">
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[0.85fr_1.15fr]">
        <ImageCurtainReveal
          className="min-h-[300px] lg:min-h-[430px]"
          direction="right"
        >
          <Image
            src={pickImage(section.primary_media?.url)}
        ...
        <AboutReveal
          variant="right"
          className="flex items-center px-5 py-14 sm:px-8 lg:px-12 xl:px-16"
        >
          <div>
            <Quote className="h-11 w-11 text-secondary" aria-hidden />
            <blockquote className="mt-6 max-w-3xl font-[family-name:var(--font-display)] text-3xl font-normal tracking-tight leading-tight sm:text-4xl">
              {section.heading}
            </blockquote>
            {section.body ? (
      ...
      <AboutReveal className="mx-auto max-w-7xl">
        ...
        <div className="mx-auto mt-10 grid max-w-5xl gap-4 md:grid-cols-2">
          {documents.map((document) => (
            <article
              key={document.id}
              className="group flex min-h-52 flex-col rounded-2xl border border-primary/15 bg-[color-mix(in_srgb,hsl(var(--primary))_6%,white)] p-6 transition-[border-color,box-shadow] duration-200 hover:border-secondary hover:shadow-lg"
            >
              <div className="flex items-start justify-between gap-4">
                ...
                {document.is_featured ? (
                  <span className="text-[0.65rem] font-bold uppercase tracking-[0.18em] text-secondary">
                    Featured
                  </span>
                ) : null}
              </div>
...
function DocumentSummary({
  ...
  page: PublicInstitutionalPage;
  promise?: PublicInstitutionalSection;
}) {
  ...
  return (
    <section className="relative z-10 bg-[#fffaf0] px-5 py-6 sm:px-8 lg:px-10">
      <AboutReveal
        className="mx-auto grid max-w-7xl overflow-hidden md:grid-cols-[auto_1fr_auto] md:items-center"
        variant="scale"
      >
        <div className="flex h-28 w-28 items-center justify-center rounded-2xl border-4 border-double border-primary/50 bg-white px-7 text-primary">
          {charter ? (
        ...
        <div className="px-6 py-6 lg:px-8">
          ...
          <h2 className="mt-2 font-[family-name:var(--font-display)] text-3xl font-normal tracking-tight text-primary">
            {charter
              ? promise?.heading || "Our Promise to You"
              : page.reporting_period_label ||
                "Our current strategic direction"}
          </h2>
          {charter && (promise?.body || promise?.summary) ? (
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              {promise?.body || promise?.summary}
            </p>
          ) : null}
        ...
        {href ? (
          <div className="px-6 pb-6 md:pb-0 md:pr-8">
            <Link
              href={href}
              className="inline-flex min-h-20 items-center gap-3 border border-secondary bg-white px-6 py-4 text-sm font-bold text-primary transition-colors duration-200 hover:bg-primary hover:text-white active:scale-[0.98]"
            >
...
function ClosingCta({ type }: { type: PageType }) {
  ...
  return (
    <section className="relative overflow-hidden bg-primary px-5 py-14 text-white sm:px-8 lg:px-10 lg:py-16">
      <div
        className="absolute -right-24 -top-24 h-72 w-72 rounded-2xl border border-white/10"
        aria-hidden
      />
      <div
        className="absolute -bottom-20 right-24 h-48 w-48 rounded-2xl border border-secondary/20"
        aria-hidden
      />
      <AboutReveal className="relative mx-auto flex max-w-7xl flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h2 className="mt-3 max-w-3xl font-[family-name:var(--font-display)] text-3xl font-normal tracking-tight leading-tight sm:text-4xl">
            {heading}
          </h2>
        ...
        <div className="flex shrink-0 flex-wrap gap-3">
          {links.map((link, index) => (
            <Link
              key={link.href}
              href={link.href}
              className={`inline-flex min-h-12 items-center gap-2 px-5 py-3 text-sm font-bold transition-[transform,background-color] duration-200 hover:-translate-y-0.5 active:scale-[0.98] motion-reduce:transform-none ${index === 0 ? "bg-secondary text-foreground hover:bg-amber-400" : "border border-white/60 text-white hover:bg-white/10"}`}
            >
...
export function InstitutionalDocumentPage({
  ...
  // The hero band is a fixed campus landmark now, but the picker is stateful:
  // claim the hero's slot anyway so the section images keep their old order.
  pickImage(page.hero_media?.url);
  const isCharter = page.page_type === "service_charter";
  const promise = isCharter
    ? page.sections.find((section) => section.section_type === "narrative")
    : undefined;
  const primaryHref = documentHref(page.primary_document);
  ...
  return (
    <main className="bg-white">
      ...
      {page.sections
        .filter((section) => !(isCharter && section.id === promise?.id))
        .map((section) => (
          <StructuredSection
            key={section.id}
            section={section}
