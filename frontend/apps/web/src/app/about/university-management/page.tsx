...
function LeadershipCard({ member, featured = false, compact = false }: { member: BoardMember; featured?: boolean; compact?: boolean }) {
  const content = (
    <article className={`group overflow-hidden rounded-2xl ring-1 ring-primary/10 bg-white text-center transition-[transform,box-shadow] duration-200 hover:-translate-y-1 hover:shadow-lg ${featured ? "w-[280px]" : compact ? "w-full max-w-[205px]" : "w-full max-w-[235px]"}`}>
      <ImageCurtainReveal className="aspect-[4/3]" direction={featured ? "right" : "left"}><PublicImage src={member.photoUrl} alt={`${member.name}, ${member.role}`} ratio="profile" className="h-full w-full" sizes={featured ? "280px" : "235px"} imageClassName="object-cover object-top transition-transform duration-500 motion-safe:group-hover:scale-[1.03]" /></ImageCurtainReveal>
      <div className={featured ? "min-h-[92px] px-4 py-4" : "min-h-[76px] px-3 py-3"}><h3 className={`font-[family-name:var(--font-display)] font-normal tracking-tight leading-tight text-foreground ${featured ? "text-lg" : compact ? "text-sm" : "text-base"}`}>{member.name}</h3><p className="mt-1.5 text-[11px] font-bold uppercase tracking-[0.08em] text-secondary">{member.role}</p></div>
    </article>
  );
  return member.profileHref ? <Link href={member.profileHref} className="block rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">{content}</Link> : content;
}
          ...
          <div className="relative grid w-full grid-cols-2 justify-items-center gap-4 pt-5">
            ...
            {reports.map((member) => (
              <div key={`${member.name}-${member.role}`} className="relative flex w-full justify-center pt-1 before:absolute before:-top-5 before:h-5 before:w-px before:bg-secondary/70">
                <span aria-hidden className="absolute -top-[1.42rem] h-2 w-2 rounded-2xl border-2 border-white bg-secondary shadow-sm" />
      ...
      <AboutPageLenis>
        <section className="relative isolate overflow-hidden bg-brand-overlay text-white">
          ...
          <div className="relative mx-auto min-h-[380px] w-full max-w-7xl px-5 py-8 sm:px-8 lg:px-10">
            ...
            <div className="mt-10 max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.18em] text-secondary">Executive leadership</p><h1 className="mt-3 font-[family-name:var(--font-display)] text-4xl font-normal tracking-tight leading-tight text-white sm:text-5xl lg:text-6xl">University <em className="italic">Management</em></h1><p className="mt-5 max-w-2xl text-base leading-7 text-white/85">{boardDescription}</p></div>
          </div>
        </section>
        ...
        <section className="bg-white px-5 py-12 ksu-section-pad sm:px-8 lg:px-10 lg:py-16">
          <AboutReveal className="mx-auto w-full max-w-7xl" variant="up">
            <div className="text-center"><p className="text-xs font-bold uppercase tracking-[0.18em] text-secondary">Executive leadership</p><h2 className="mt-2 font-[family-name:var(--font-display)] text-2xl font-normal tracking-tight uppercase text-primary">Our Leadership <em className="italic">Structure</em></h2><div className="mx-auto mt-2 h-0.5 w-10 bg-secondary" /></div>

            {viceChancellor ? <div className="mt-6 flex justify-center"><LeadershipCard member={viceChancellor} featured /></div> : <p className="mt-8 ksu-mt-compact rounded-2xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Management profiles have not been published yet.</p>}
              ...
              <>
                <div className="hidden md:block">
                  ...
                  <div className="relative mx-auto max-w-5xl">
                    <span aria-hidden className="absolute left-1/4 right-1/4 top-0 h-px bg-secondary/70" />
                    <span aria-hidden className="absolute left-1/2 top-[-4px] h-2 w-2 -translate-x-1/2 rounded-2xl border-2 border-white bg-secondary shadow-sm" />
            ...
            <div className="mt-10 grid gap-5 rounded-2xl bg-[color-mix(in_srgb,hsl(var(--primary))_6%,white)] p-6 sm:grid-cols-[3.5rem_1fr_auto] sm:items-center"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary text-secondary"><Award className="h-6 w-6" aria-hidden /></span><div><h2 className="font-[family-name:var(--font-display)] text-xl font-normal tracking-tight text-primary">Committed to <em className="italic">Excellence</em></h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Our management team advances academic quality, innovation and dependable service across the University.</p></div><Link href="/about/university-council" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-3 text-sm font-bold text-white hover:bg-primary/90 active:scale-[0.98]">Our Governance <ArrowRight className="h-4 w-4" aria-hidden /></Link></div>
          </AboutReveal>
        </section>
      </AboutPageLenis>
    </PageShell>
