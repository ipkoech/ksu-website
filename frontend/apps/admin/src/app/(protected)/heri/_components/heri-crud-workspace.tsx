...
type Field = { name: string; label: string; type?: "text" | "textarea" | "richtext" | "select" | "boolean" | "number" | "date"; required?: boolean; options?: string[] };
...
export function HeriCrudWorkspace({ config }: { config: Config }) {
  ...
  const transitionSelected = async (nextStatus: string) => { if (!canWrite || selectedIds.size === 0) return; try { await Promise.all(Array.from(selectedIds, (id) => request(`/admin/${config.resource}/${id}`, { method: "PATCH", body: JSON.stringify({ status: nextStatus }) }))); toast.success(`${selectedIds.size} workflow records updated`); await load(); } catch (reason) { toast.error(reason instanceof Error ? reason.message : "Unable to update workflow"); } };
