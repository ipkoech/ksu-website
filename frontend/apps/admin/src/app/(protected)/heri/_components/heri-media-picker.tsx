...
export function HeriMediaPicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  const upload = async (file?: File) => { if (!file) return; setUploading(true); try { const asset = await heriRequest<MediaAsset>(`/admin/media/upload?folder=heri&filename=${encodeURIComponent(file.name)}`, { method: "POST", body: file, headers: { "Content-Type": file.type || "application/octet-stream" } }); void revalidatePublicContent("heri", "media"); onChange(asset.public_url ?? asset.id); toast.success("Media uploaded"); await load(); } catch (reason) { toast.error(reason instanceof Error ? reason.message : "Unable to upload media"); } finally { setUploading(false); if (inputRef.current) inputRef.current.value = ""; } };
