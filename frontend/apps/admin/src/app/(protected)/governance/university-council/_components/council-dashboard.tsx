...
import { councilGovernanceProfile, type CouncilDashboard as CouncilDashboardStats, type GovernanceWorkspaceProfile } from "@/lib/api/organization";
...
const actionItems = (profile: GovernanceWorkspaceProfile) => [
  {
    label: `Add ${profile.memberSingular}`,
  ...
  },
  {
    ...
    description: `Review the public ${profile.badgeLabel} page before publishing.`,
    icon: Eye,
    tab: "preview",
  },
  {
  ...
  },
  {
    ...
    description: `Review former ${profile.badgeLabel} appointments.`,
    icon: Archive,
    tab: "archive",
  },
];

export function CouncilDashboard({ profile = councilGovernanceProfile }: { profile?: GovernanceWorkspaceProfile }) {
  const [activeTab, setActiveTab] = useState("members");
  const dashboardQuery = useQuery({
    queryKey: ["governance", profile.key, "dashboard"],
    queryFn: () => profile.api.dashboard(),
  });
      ...
      <div className="flex flex-col gap-6 p-4 sm:p-6">
          ...
          <div className="flex flex-col gap-4 border-b p-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="secondary">{profile.badgeLabel}</Badge>
                <Badge variant="outline">Workflow managed</Badge>
              </div>
              <h2 className="mt-3 text-lg font-semibold">{profile.overviewTitle}</h2>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          ...
          <TabsContent id="members" value="members">
            <CouncilMemberEditor profile={profile} />
          </TabsContent>
          <TabsContent id="order" value="order">
            <CouncilOrderManager profile={profile} />
          </TabsContent>
          <TabsContent id="page-content" value="page-content">
            <CouncilPageContentEditor profile={profile} />
          </TabsContent>
          <TabsContent id="preview" value="preview">
            <CouncilPreview profile={profile} />
          </TabsContent>
          <TabsContent id="archive" value="archive">
            <CouncilMemberEditor profile={profile} archivedOnly />
          </TabsContent>
        </Tabs>
      </div>
    </div>
