...
import { BookOpen, HelpCircle, Library, Ticket, Users } from "lucide-react";
...
const libraryAreas = [
  {
    title: "Catalog",
    description: "Resource lists require a selected library branch in the backend contract.",
    href: "/library/catalog",
    icon: Library,
  },
  {
    title: "Circulation",
    description: "Loan and reservation workflows are served by the library API.",
...
export function LibraryDashboardClient() {
  const branches = useQuery({
    ...
    queryFn: () => libraryApi.get<CountResponse>("/api/v1/library/branches", { ...countParams, active_only: true }),
  });
  const loans = useQuery({
    queryKey: ["library", "loans", "count"],
    queryFn: () => libraryApi.get<CountResponse>("/api/v1/library/loans", countParams),
  });
  const inquiries = useQuery({
    queryKey: ["library", "inquiries", "count"],
    queryFn: () => libraryApi.get<CountResponse>("/api/v1/library/inquiries", countParams),
  });
  const tickets = useQuery({
    queryKey: ["library", "tickets", "count"],
    queryFn: () => libraryApi.get<CountResponse>("/api/v1/library/tickets", countParams),
  });
  ...
  const stats = [
    ...
    {
      title: "Inquiries",
      value: formatCount(inquiries.data, inquiries.isLoading, inquiries.isError),
      ...
      <div className="space-y-6 p-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Navigation is aligned to the service modules currently visible in the admin shell.</CardDescription>
          </CardHeader>
