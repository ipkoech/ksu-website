...
export default function LibraryStaffPage() {
  ...
  const canManageStaff = hasScope("library.manage_staff") || hasScope("library:write");
  const [libraryId, setLibraryId] = useState("");
  const [editing, setEditing] = useState<LibraryGenericRecord | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<LibraryGenericRecord | null>(null);
  ...
  const staffQuery = useQuery({
    ...
    queryFn: () => libraryServiceApi.staff.list({ library_id: libraryId, page: 1, per_page: 100 }),
    enabled: !!libraryId,
  });
  ...
  const createStaff = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["library", "staff", libraryId] }),
  });
  const updateStaff = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: Record<string, any> }) => libraryServiceApi.staff.update(id, payload),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["library", "staff", libraryId] }),
  });
  ...
  const submit = async () => {
    ...
    const payload = editing ? editablePayload : { ...editablePayload, library_id: libraryId, person_id: values.person_id };
  ...
  return (
    <div>
      <PageHeader title="Library Staff" description="Manage branch-scoped staff listings and public librarian profiles." backHref="/library" />
      <div className="grid gap-6 p-6 xl:grid-cols-[1fr_420px]">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {staffQuery.isLoading ? (
              ...
              <p className="rounded-lg border bg-background p-4 text-sm text-muted-foreground">No staff records found for this branch.</p>
            ) : (
                  ...
                  <div key={member.id} className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="font-medium">{member.job_title || member.role || member.person_id}</p>
                        <Badge variant="outline">{member.role || "staff"}</Badge>
                        {typeof member.is_active === "boolean" ? <Badge variant={member.is_active ? "default" : "secondary"}>{member.is_active ? "Active" : "Inactive"}</Badge> : null}
                      </div>
                      <p className="mt-1 text-sm text-muted-foreground">{[member.department, member.specialization, member.person_id].filter(Boolean).join(" · ") || "No staff metadata"}</p>
                    </div>
                    <div className="flex gap-2">
                      {canManageStaff ? (
                        <>
                          <Button variant="outline" size="sm" onClick={() => edit(member)}><Edit className="mr-2 h-4 w-4" />Edit</Button>
                          <Button variant="outline" size="sm" className="text-destructive" onClick={() => setDeleteTarget(member)}><Trash2 className="mr-2 h-4 w-4" />Delete</Button>
                        </>
                      ) : null}
                    </div>
                  </div>
        ...
        <Card>
          <CardHeader><CardTitle>{editing ? "Edit Staff Member" : "Create Staff Member"}</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            {!editing ? (
              <div className="space-y-2">
                <label className="text-sm font-medium">Person ID *</label>
                <Input disabled={!canManageStaff} value={values.person_id ?? ""} onChange={(event) => setValues((current) => ({ ...current, person_id: event.target.value }))} />
              </div>
            ) : null}
            {[
              ["job_title", "Job Title"],
            ...
            <div className="space-y-2">
              ...
              <RichTextEditor disabled={!canManageStaff} toolbar="simple" minHeight="170px" value={values.bio ?? ""} onChange={(bio) => setValues((current) => ({ ...current, bio }))} />
            </div>
            {(["is_public", "is_active"] as const).map((name) => (
              <div key={name} className="flex items-center justify-between rounded-lg border p-3">
                <span className="text-sm font-medium">{name.replace(/_/g, " ")}</span>
                <Switch disabled={!canManageStaff} checked={Boolean(values[name])} onCheckedChange={(checked) => setValues((current) => ({ ...current, [name]: checked }))} />
              </div>
            ))}
            <div className="flex gap-2">
              <Button onClick={submit} disabled={!canManageStaff || createStaff.isPending || updateStaff.isPending}>{createStaff.isPending || updateStaff.isPending ? "Saving..." : editing ? "Save Changes" : "Create Staff"}</Button>
