...
export const contentTabs = [
  ...
  { label: "Stories", href: "/research/content/blogs" },
  ...
  return (
    <ResearchWorkspaceHeader
      metrics={[
        {
          title: "Published",
          queryKey: ["research", "content", "metrics", "published"],
          queryFn: () =>
            newsApi.listAdmin({
              page: 1,
              per_page: 1,
        ...
        {
          title: "Drafts",
          queryKey: ["research", "content", "metrics", "drafts"],
          queryFn: () =>
            newsApi.listAdmin({
              page: 1,
              per_page: 1,
        ...
        {
          title: "Scheduled",
          queryKey: ["research", "content", "metrics", "scheduled"],
          queryFn: () =>
            newsApi.listAdmin({
              page: 1,
              per_page: 1,
        ...
        {
          title: "Needs Review",
          queryKey: ["research", "content", "metrics", "review"],
          queryFn: () =>
            newsApi.listAdmin({
              page: 1,
              per_page: 1,
        ...
        },
        {
          ...
          queryKey: ["research", "content", "metrics", "recent"],
          queryFn: () => slidersApi.listGroups({ scope_type: "research" }),
...
export const contentColumns: Array<
  EditableRecordColumn<Record<string, any> & { id: string }>
> = [
  {
    key: "title",
    label: "Title",
  ...
  },
  {
    ...
    render: (record) => (
      <StatusBadge
        value={record.status ?? (record.is_published ? "published" : "draft")}
      />
    ),
  },
  {
    key: "category",
    label: "Category / Type",
    className: "hidden min-w-[160px] lg:table-cell",
    render: (record) => (
      <span>
        {record.category ?? record.event_type ?? record.location ?? "General"}
      </span>
    ),
  },
  {
    ...
    render: (record) => (
      <DateValue
        value={record.published_at ?? record.publish_date ?? record.start_date}
      />
    ),
  },
  {
  ...
  },
  {
    ...
    label: "Updated",
    className: "hidden w-[150px] xl:table-cell",
    render: (record) => <DateValue value={record.updated_at} />,
  },
];
