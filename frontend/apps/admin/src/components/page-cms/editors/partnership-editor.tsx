...
import { Button, Input, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@ksu/ui/components";
...
export function PartnershipEditor(props: SectionEditorProps) {
  ...
  const cta = editor.draft.items.find((item) => item.item_type === "cta") ?? { item_type: "cta" as const, display_order: 20, is_enabled: true, content: {} };
  ...
  const updateCta = (key: "source" | "label" | "url", value: string) => editor.setDraft((current) => {
    const currentCta = current.items.find((item) => item.item_type === "cta") ?? cta;
    const content = { ...(currentCta.content ?? {}), source: key === "source" ? value : String((currentCta.content as Record<string, unknown> | null)?.source ?? "manual") };
    const nextCta = { ...currentCta, content, title: key === "label" ? value || null : currentCta.title, cta_label: key === "label" ? value || null : currentCta.cta_label, cta_url: key === "url" ? value || null : currentCta.cta_url };
    return { ...current, items: [...current.items.filter((item) => item.item_type !== "cta"), nextCta] };
  });
  ...
  return <div className="space-y-5"><SharedSectionFields draft={editor.draft} setDraft={editor.setDraft} readOnly={props.readOnly} /><fieldset disabled={props.readOnly} className="space-y-4 border-t border-border pt-4"><SourceRecordPicker sourceType="research_partner" layoutVariant="featured_partnership" scopeType={props.section.scope_type} scopeId={props.section.scope_id} label="Research partner" value={sourcePickerValue(props.section, "research_partner", partner)} onChange={(value) => editor.setDraft((current) => ({ ...current, items: [...current.items.filter((item) => item.item_type !== "reference" || item.source_type !== "research_partner"), ...(value ? [createReferenceItem("research_partner", value.sourceId, 10)] : [])] }))} disabled={props.readOnly} /><div className="grid gap-4 md:grid-cols-3"><label className="space-y-2 text-sm font-medium">CTA source<Select value={String((cta.content as Record<string, unknown> | null)?.source ?? "manual")} onValueChange={(value) => updateCta("source", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="manual">Manual</SelectItem><SelectItem value="partner_website">Partner website</SelectItem><SelectItem value="generated_detail_page">Generated detail page</SelectItem></SelectContent></Select></label><label className="space-y-2 text-sm font-medium">CTA label<Input value={cta.cta_label ?? ""} onChange={(event) => updateCta("label", event.target.value)} /></label><label className="space-y-2 text-sm font-medium">CTA URL<Input value={cta.cta_url ?? ""} placeholder="https:// or /path" onChange={(event) => updateCta("url", event.target.value)} /></label></div></fieldset><div className="grid gap-5 border-t border-border pt-4 xl:grid-cols-2"><RepeatableRows name="pillar" values={rows(editor.draft.settings.pillars)} onChange={(value) => setRows("pillars", value)} readOnly={props.readOnly} /><RepeatableRows name="opportunity" values={rows(editor.draft.settings.opportunities)} onChange={(value) => setRows("opportunities", value)} readOnly={props.readOnly} /></div><SectionMediaRoles section={props.section} definition={props.definition} readOnly={props.readOnly} /><EditorActions section={props.section} dirty={editor.dirty} isSaving={editor.isSaving} saveError={editor.saveError} readOnly={props.readOnly} onReset={editor.reset} onSave={() => void editor.save(validate)} /></div>;
}
