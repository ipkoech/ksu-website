...
export function PillarGridEditor(props: SectionEditorProps) {
  ...
  const visiblePillars: SectionItemPayload[] = pillars;
  ...
  return <div className="space-y-5"><SharedSectionFields draft={editor.draft} setDraft={editor.setDraft} readOnly={props.readOnly} /><section aria-label="Pillar rows" className="space-y-3 border-t border-border pt-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="text-sm font-semibold">Pillars</h3><p className="mt-1 text-sm text-muted-foreground">Keep two to six rows.</p></div><div className="flex gap-2">{!props.readOnly && !visiblePillars.length ? <Button type="button" variant="outline" size="sm" onClick={() => updatePillars(defaultPillars())}>Use recommended pillars</Button> : null}<Button type="button" variant="outline" size="sm" disabled={props.readOnly || visiblePillars.length >= MAX_PILLARS} onClick={() => updatePillars([...visiblePillars, { item_type: "card", title: "", body_text: "", display_order: (visiblePillars.length + 1) * 10, is_enabled: true }])}><Plus />Add pillar</Button></div></div>{visiblePillars.map((pillar, index) => <div key={pillar.id ?? index} className="grid gap-3 border-b border-border pb-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)_auto]"><label className="space-y-2 text-sm font-medium">Title<Input value={pillar.title ?? ""} disabled={props.readOnly} onChange={(event) => updatePillars(visiblePillars.map((row, rowIndex) => rowIndex === index ? { ...row, title: event.target.value } : row))} /></label><label className="space-y-2 text-sm font-medium">Description<Textarea rows={2} value={pillar.body_text ?? ""} disabled={props.readOnly} onChange={(event) => updatePillars(visiblePillars.map((row, rowIndex) => rowIndex === index ? { ...row, body_text: event.target.value } : row))} /></label><div className="flex items-end gap-1"><Button type="button" variant="ghost" size="icon" aria-label="Move pillar up" disabled={props.readOnly || index === 0} onClick={() => updatePillars(moveRow(visiblePillars, index, -1))}><ArrowUp /></Button><Button type="button" variant="ghost" size="icon" aria-label="Move pillar down" disabled={props.readOnly || index === visiblePillars.length - 1} onClick={() => updatePillars(moveRow(visiblePillars, index, 1))}><ArrowDown /></Button><Button type="button" variant="ghost" size="icon" aria-label="Remove pillar" disabled={props.readOnly || visiblePillars.length <= MIN_PILLARS} onClick={() => updatePillars(visiblePillars.filter((_, rowIndex) => rowIndex !== index))}><Trash2 /></Button></div></div>)}</section><EditorActions section={props.section} dirty={editor.dirty} isSaving={editor.isSaving} saveError={editor.saveError} readOnly={props.readOnly} onReset={editor.reset} onSave={() => void editor.save(validate)} /></div>;
}
