...
function richTextValue(value: unknown) {
  const clean = sanitizeRichText(String(value ?? ""));
  return clean ? clean : null;
}
      ...
      <section className="mb-5 overflow-hidden rounded-2xl border bg-[radial-gradient(circle_at_top_right,rgba(217,119,6,0.18),transparent_32%),linear-gradient(135deg,rgba(255,255,255,0.98),rgba(248,250,252,0.88))] p-4 shadow-sm dark:bg-[radial-gradient(circle_at_top_right,rgba(245,158,11,0.14),transparent_32%),linear-gradient(135deg,rgba(15,23,42,0.98),rgba(2,6,23,0.9))]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {canManage ? <Button type="button" onClick={() => setEditionEditor("new")}><Plus className="size-4" />Create reporting edition</Button> : null}
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[19rem_minmax(0,1fr)]">
        <Card className="h-fit">
          <CardHeader className="space-y-3">
            <div>
              <CardTitle>Reporting editions</CardTitle>
              <CardDescription>Select a year to manage its annual groups.</CardDescription>
            </div>
            {canManage ? <Button type="button" size="sm" onClick={() => setEditionEditor("new")}><Plus className="size-4" />Create edition</Button> : null}
          </CardHeader>
    ...
    <Card>
      ...
      <CardContent className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl bg-muted/40 p-4">
          ...
          <RichTextRenderer content={edition.introduction} className="mt-2 prose-sm text-sm leading-6" emptyFallback={<p className="mt-2 text-sm leading-6 text-muted-foreground">No introduction supplied.</p>} />
        </div>
        <div className="rounded-xl bg-muted/40 p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Methodology</p>
          <RichTextRenderer content={edition.methodology_note} className="mt-2 prose-sm text-sm leading-6" emptyFallback={<p className="mt-2 text-sm leading-6 text-muted-foreground">No methodology note supplied.</p>} />
        </div>
      </CardContent>
    </Card>
  );
  ...
  return (
    <section className="overflow-hidden rounded-2xl border">
      <div className="flex flex-col gap-3 border-b bg-muted/25 p-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{group.heading}</h3><Badge variant="outline">{factKindTitle(kind)}</Badge><WorkflowBadge status={group.workflow_status} />{group.image_id ? <Badge variant="secondary">Media attached</Badge> : null}</div>
          <RichTextRenderer content={group.summary} className="mt-1 prose-sm text-sm text-muted-foreground" emptyFallback={<p className="mt-1 text-sm text-muted-foreground">Order {group.display_order}</p>} />
        </div>
...
function FactItemRow({ item, canManage, onEdit, onChanged }: { item: FactItem; canManage: boolean; onEdit: () => void; onChanged: () => Promise<void> }) {
  ...
  return (
    <div className="grid gap-3 p-4 md:grid-cols-[minmax(0,1fr)_minmax(10rem,0.45fr)_auto] md:items-center">
      <div>
        <div className="flex flex-wrap items-center gap-2"><p className="font-medium">{item.label}</p><WorkflowBadge status={item.workflow_status} />{item.is_featured ? <Badge variant="secondary">Featured</Badge> : null}</div>
        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
          {item.source_url ? <Link href={item.source_url} target={item.source_url.startsWith("http") ? "_blank" : undefined} className="font-medium text-primary underline-offset-4 hover:underline">{item.source_title || "Source evidence"}</Link> : <span>{item.source_title || "Source required before publishing"}</span>}
          {item.verified_on ? <span>Verified {item.verified_on}</span> : null}
          {item.link_url ? <Link href={item.link_url} target={item.link_url.startsWith("http") ? "_blank" : undefined} className="font-medium text-primary underline-offset-4 hover:underline">{item.link_label || "Public link"}</Link> : null}
        </div>
        {item.explanation ? <RichTextRenderer content={item.explanation} className="mt-2 prose-sm text-sm text-muted-foreground" /> : null}
      </div>
...
function EditionEditorDialog({ record, onOpenChange, onSaved }: { record: FactEdition | "new" | null; onOpenChange: (open: boolean) => void; onSaved: () => Promise<void> }) {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      const payload: FactEditionPayload = { reporting_year: Number(values.reporting_year), title: String(values.title ?? "").trim(), introduction: richTextValue(values.introduction), methodology_note: richTextValue(values.methodology_note), verified_on: nullable(values.verified_on), source_document_id: nullable(values.source_document_id), is_current: values.is_current ?? false, is_enabled: values.is_enabled ?? true };
...
function GroupEditorDialog({ state, editionId, onOpenChange, onSaved }: { state: GroupEditorState; editionId: string | null; onOpenChange: (open: boolean) => void; onSaved: () => Promise<void> }) {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      const payload: FactGroupPayload = { fact_edition_id: state.kind === "annual" ? editionId : null, slug: String(values.slug ?? "").trim(), heading: String(values.heading ?? "").trim(), summary: richTextValue(values.summary), image_id: nullable(values.image_id), image_alt_text: nullable(values.image_alt_text), display_order: Number(values.display_order ?? 100), is_enabled: values.is_enabled ?? true };
  ...
  return <Dialog open={Boolean(state)} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>{state?.record ? "Edit" : "Add"} {factKindLabel(state?.kind)} group</DialogTitle><DialogDescription>{state?.kind === "evergreen" ? "This institutional group will appear in every public edition." : "This group belongs only to the selected reporting year."}</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><TextField label="Slug" value={values.slug} disabled={Boolean(state?.record)} onChange={(value) => setValues((current) => ({ ...current, slug: value }))} /><TextField label="Display order" type="number" value={values.display_order} onChange={(value) => setValues((current) => ({ ...current, display_order: Number(value) }))} /></div><TextField label="Heading" value={values.heading} onChange={(value) => setValues((current) => ({ ...current, heading: value }))} /><RichTextField label="Summary" value={values.summary} onChange={(value) => setValues((current) => ({ ...current, summary: value }))} placeholder="Summarise this fact group for the public page." /><MediaPicker value={values.image_id ?? ""} onChange={(value) => setValues((current) => ({ ...current, image_id: value }))} mediaType="image" accept="image/*" label="Section image" /><TextField label="Image alt text" value={values.image_alt_text} onChange={(value) => setValues((current) => ({ ...current, image_alt_text: value }))} /><ToggleField label="Enabled" checked={values.is_enabled ?? true} onChange={(checked) => setValues((current) => ({ ...current, is_enabled: checked }))} /><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button type="button" disabled={mutation.isPending} onClick={() => mutation.mutate()}>{mutation.isPending ? "Saving…" : "Save group"}</Button></DialogFooter></DialogContent></Dialog>;
}
...
function ItemEditorDialog({ state, onOpenChange, onSaved }: { state: ItemEditorState; onOpenChange: (open: boolean) => void; onSaved: (groupId: string) => Promise<void> }) {
  ...
  return <Dialog open={Boolean(state)} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl"><DialogHeader><DialogTitle>{state?.record ? "Edit" : "Add"} {factKindLabel(state?.kind)} fact</DialogTitle><DialogDescription>Display values may be numeric or descriptive. Source and verification are required before publication.</DialogDescription></DialogHeader><div className="rounded-2xl border bg-muted/30 p-4"><p className="mb-3 text-sm font-semibold">Fact number</p><div className="grid gap-4 sm:grid-cols-2"><TextField label="Label" value={values.label} onChange={(value) => setValues((current) => ({ ...current, label: value }))} /><TextField label="Display value" value={values.display_value} onChange={(value) => setValues((current) => ({ ...current, display_value: value }))} /></div><div className="mt-4 grid gap-4 sm:grid-cols-4"><TextField label="Numeric value" type="number" value={values.numeric_value} onChange={(value) => setValues((current) => ({ ...current, numeric_value: value }))} /><TextField label="Prefix" value={values.prefix} onChange={(value) => setValues((current) => ({ ...current, prefix: value }))} /><TextField label="Suffix" value={values.suffix} onChange={(value) => setValues((current) => ({ ...current, suffix: value }))} /><TextField label="Unit" value={values.unit} onChange={(value) => setValues((current) => ({ ...current, unit: value }))} /></div></div><RichTextField label="Explanation" value={values.explanation} onChange={(value) => setValues((current) => ({ ...current, explanation: value }))} placeholder="Add optional context, caveats or calculation notes." /><div className="rounded-2xl border bg-muted/30 p-4"><p className="mb-3 text-sm font-semibold">Links and source evidence</p><div className="grid gap-4 sm:grid-cols-2"><TextField label="Source title" value={values.source_title} onChange={(value) => setValues((current) => ({ ...current, source_title: value }))} /><TextField label="Verification date" type="date" value={values.verified_on} onChange={(value) => setValues((current) => ({ ...current, verified_on: value }))} /><TextField label="Source URL" value={values.source_url} onChange={(value) => setValues((current) => ({ ...current, source_url: value }))} placeholder="https://… or /internal-path" /><TextField label="Public link URL" value={values.link_url} onChange={(value) => setValues((current) => ({ ...current, link_url: value }))} placeholder="https://… or /internal-path" /><TextField label="Public link label" value={values.link_label} onChange={(value) => setValues((current) => ({ ...current, link_label: value }))} /><TextField label="Icon key" value={values.icon_key} onChange={(value) => setValues((current) => ({ ...current, icon_key: value }))} /></div></div><div className="grid gap-4 sm:grid-cols-[1fr_1fr_1fr]"><TextField label="Display order" type="number" value={values.display_order} onChange={(value) => setValues((current) => ({ ...current, display_order: Number(value) }))} /><ToggleField label="Featured" checked={values.is_featured ?? false} onChange={(checked) => setValues((current) => ({ ...current, is_featured: checked }))} /><ToggleField label="Enabled" checked={values.is_enabled ?? true} onChange={(checked) => setValues((current) => ({ ...current, is_enabled: checked }))} /></div><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button type="button" disabled={mutation.isPending} onClick={() => mutation.mutate()}>{mutation.isPending ? "Saving…" : "Save fact"}</Button></DialogFooter></DialogContent></Dialog>;
}
...
function RichTextField({ label, value, onChange, placeholder }: { label: string; value: unknown; onChange: (value: string) => void; placeholder?: string }) { return <label className="space-y-2 text-sm font-medium"><span>{label}</span><RichTextEditor value={String(value ?? "")} onChange={onChange} toolbar="simple" minHeight="9rem" maxHeight="22rem" placeholder={placeholder} /></label>; }
