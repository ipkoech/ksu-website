...
export default async function OurWorkPage() {
  ...
  const impactMetrics = metricData.map(({ id, label, value, unit, description }) => ({
    id,
    label,
    value,
    unit,
  ...
  return (
    <SiteShell>
      <main className="bg-white">
        ...
        <PublicImpactMetrics metrics={impactMetrics} />
        ...
        <section className="mx-auto max-w-7xl px-6 py-16">
          ...
          <PublicResearchPortfolio themes={themeData} projects={projectData.slice(0, 6)} publications={publicationData.slice(0, 6)} />
        </section>

        <PublicOpportunities items={opportunityItems} />
