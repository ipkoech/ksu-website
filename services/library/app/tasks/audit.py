"""Library-owned durable audit capture and bounded forwarding to Main."""
...
async def _relay_pending() -> int:
    return await relay_audit_batch(
        AsyncSessionLocal, audit_relay, _persist_in_main, owner_key="library.audit.relay"
    )


@celery_app.task(name="library.audit.relay", ignore_result=True, soft_time_limit=30, time_limit=45)
...
@celery_app.task(name="library.audit.observe", ignore_result=True, soft_time_limit=10, time_limit=15)
