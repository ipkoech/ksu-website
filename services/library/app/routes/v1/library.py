...
@cached_public(timeout=60, vary_on=("fields", "include"))
async def get_library(
    ...
    selector = FieldSelector(Library, fields, always_include={"id"})
    library = await svc.get_library(db, library_id, load_options=selector.load_options)
...
async def get_library_hours(
    ...
    hours = await svc.get_library_hours(db, library_id)
    return success(data=hours)
...
async def toggle_external_link(
    ...
    link = await svc.toggle_external_link(db, link_id, is_active)
    return success(data=link)
