...
@pytest.mark.asyncio
async def test_heri_app_audits_the_committed_http_outcome(monkeypatch):
    from app import main

    url = os.environ.get("KSU_TEST_DATABASE_URL")
    if not url:
        pytest.skip("disposable KSU_TEST_DATABASE_URL required")
    schema = "heri_http_audit_" + uuid.uuid4().hex
    engine = create_async_engine(url, pool_size=1, max_overflow=0,
                                 execution_options={"schema_translate_map": {"heri": schema}})
    ...
    try:
        ...
        metrics.gauge.assert_any_call("audit.relay.ready_age", 0, tags={"service": "heri"})
        metrics.gauge.assert_any_call("audit.relay.retained_failure", 0, tags={"service": "heri"})
