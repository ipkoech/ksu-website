...
@pytest.mark.asyncio
async def test_heri_public_cache_invalidation_is_commit_gated(monkeypatch):
    from app import main

    invalidate = AsyncMock()
    monkeypatch.setattr(main, "invalidate_prefix", invalidate)

    await main._after_response(_request("PATCH", "/api/v1/admin/news/1"), Response(status_code=200))
    await main._after_response(_request("PATCH", "/api/v1/admin/news/2"), Response(status_code=409))
