...
from ...models.content import Event, NewsArticle
from ...schemas.admin_content import EventAdminResponse, EventCreate, NewsAdminResponse, NewsCreate, NewsUpdate, TransitionRequest
from ksu_common.schemas.pagination import PaginatedResponse
from ...services.audit import record_audit
from ...services.workflow import lock_record, require_editable, transition_record

router = APIRouter(prefix="/admin", tags=["HERI Admin Content"])


@router.get("/news", response_model=PaginatedResponse[NewsAdminResponse])
async def list_news(page: int = Query(1, ge=1), per_page: int = Query(25, ge=1, le=100), search: str | None = Query(None, min_length=1, max_length=120), status_filter: str | None = Query(None, alias="status"), db: AsyncSession = Depends(get_db), _: TokenPayload = Depends(require_permission("heri.content.read"))):
    ...
    if status_filter:
        filters.append(NewsArticle.status == status_filter)
    if search:
        filters.append(NewsArticle.title.ilike(f"%{search}%"))
...
async def update_news(article_id: UUID, payload: NewsUpdate, request: Request, db: AsyncSession = Depends(get_db), user: TokenPayload = Depends(require_permission("heri.content.write"))):
    record = await lock_record(db, NewsArticle, article_id)
...
async def transition_news(article_id: UUID, payload: TransitionRequest, request: Request, db: AsyncSession = Depends(get_db), user: TokenPayload = Depends(get_current_user)):
    return await transition_record(
        db, NewsArticle, article_id, payload.status, actor=user,
        entity_type="news_article", note=payload.note,
        scheduled_at=payload.scheduled_at,
        ip_address=request.client.host if request.client else None,
    )


@router.get("/events", response_model=PaginatedResponse[EventAdminResponse])
...
@router.post("/events", response_model=EventAdminResponse, status_code=status.HTTP_201_CREATED)
