"""Off-request audit persistence for the HERI Africa service.

HERI was the only service that never recorded a request audit trail. The row is
built inside the request (the Request and its body are gone afterwards) but the
write happens in a worker, so adding the trail does not add request latency.
"""
...
@celery_app.task(name="heri.audit.persist", ignore_result=True)
