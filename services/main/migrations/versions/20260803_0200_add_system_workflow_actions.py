"""Widen content_workflow_logs action CHECK to include system_publish/system_expire."""
from __future__ import annotations

from alembic import op

revision = "20260803_0200"
...
def downgrade() -> None:
    op.execute(
        "DELETE FROM content_workflow_logs WHERE action IN ('system_publish', 'system_expire')"
    )
    op.drop_constraint("ck_content_workflow_logs_action", "content_workflow_logs", type_="check")
    op.create_check_constraint(
