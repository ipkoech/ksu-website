...
async def create_academic_calendar(data: AcademicCalendarCreate, db: DbSession, _: CurrentUser):
...
@router.patch("/{calendar_id}", dependencies=[Depends(require_scope("academic:write"))])
async def update_academic_calendar(calendar_id: uuid.UUID, data: AcademicCalendarUpdate, db: DbSession, _: CurrentUser):
...
@router.delete("/{calendar_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_scope("academic:delete"))])
async def delete_academic_calendar(calendar_id: uuid.UUID, db: DbSession, _: CurrentUser):
    result = await db.execute(AcademicCalendar.active_query().where(AcademicCalendar.id == calendar_id))
    calendar = result.scalar_one_or_none()
    if calendar is None:
        raise HTTPException(status_code=404, detail="Academic calendar not found")
