...
async def _deliver_reply(message_id: uuid.UUID) -> str:
    async with AsyncSessionLocal() as db:
        ...
        subject = f"Re: {message.inquiry.subject} [{message.inquiry.reference_number}]"
        body = (
    ...
    async with AsyncSessionLocal() as db:
        ...
        enqueue_domain_event(
            db,
            event_type="school.inquiry.reply_sent",
            scope_type="school",
            scope_id=message.inquiry.school_id,
    ...
    *,
    ...
    async with AsyncSessionLocal() as db:
        ...
        enqueue_domain_event(
            db,
            event_type=(
                "school.inquiry.reply_dead_lettered"
                if dead_letter
                else "school.inquiry.reply_failed"
            ),
            scope_type="school",
            scope_id=message.inquiry.school_id,
            actor_id=message.sender_user_id,
